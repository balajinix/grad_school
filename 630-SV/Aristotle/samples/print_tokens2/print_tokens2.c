...
/**********************************************/
/* NAME:	is_end_of_character_stream    */
/* INPUT:       a pointer to char_stream      */
/* OUTPUT:      a BOOLEAN value               */
/**********************************************/
 int is_end_of_character_stream(fp)
character_stream fp;
{ char ch;
  ch = get_char(fp);
  if( ch == EOF)
...
/*************************************************/
/* NAME:	compare_token                    */
/* INPUT:       two compared tokens              */
/* OUTPUT:      a BOOLEAN value                  */
/*************************************************/
int compare_token(token1,token2)
token token1,token2;
{
 int type1,type2,equal;
 type1=token_type(token1);
 type2=token_type(token2);
 if( type1 !=type2)                    /* type1 not equal to type2 */
     return(FALSE);
 else if(type1==error || type2==error) /* either oh two is error token */
     return(FALSE);
 else if(type1 ==keyword || type1==spec_symbol) /* keyword or spec_symbol */
     { 
      equal=strcmp(token1,token2);
      if( equal ==0 )
          return(TRUE);             /* same keyword  or same special symbol */
      else 
          return(FALSE);
      }
 else
      return(TRUE);                      /* identifier or constant */
 }  
